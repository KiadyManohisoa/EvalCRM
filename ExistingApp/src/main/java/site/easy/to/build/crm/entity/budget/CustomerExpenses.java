...
public class CustomerExpenses {
    ...
    public void saveSelf(Connection co) throws Exception {
        ...
        String query = "insert into customer_expenses (amount, expense_date, ticket_id, lead_id) "
                +
                "values (?, ?, ?, ?)";
        try {
            st = co.prepareStatement(query);
            st.setDouble(1, this.getAmount());
            st.setTimestamp(2, Timestamp.valueOf(this.getExpense_date()));
            if(this.getTicket()!=null) {
                st.setInt(3, this.getTicket().getTicketId());
                st.setNull(4, Types.INTEGER);
            }
            else if(this.getLead()!=null) {
                st.setNull(3, Types.INTEGER);
                st.setInt(4, this.getLead().getLeadId());
            }
            st.executeUpdate();
        } 
        catch(Exception e) {
